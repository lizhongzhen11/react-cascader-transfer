...
/**
 * @description 根据已选中的value去数据源中拿到对应的数据
 */
 ...
 */
export const changeAncestorsChecked = (items: DataProps[][], dataSource: DataProps[], rowData: DataProps, selected: DataProps[]) => {
   ...
   */
  if (level === 0) {
    rowData.checked && handleSelected(selected, rowData)
    !rowData.checked && rowData.children && rowData.children.every(item => !item.checked) && handleSelected(selected, rowData, 'rootCancel')
    // !rowData.checked && handleSelected(selected, rowData, 'rootCancel')
    return
  }
  /** checkbox状态改变，更新selected数据 */
  /** 该节点选中，但是其兄弟节点并未全部选中，父层也就未选中，所以将该节点数据放入selected */
  if (!parent.checked && rowData.checked) {
    handleSelected(selected, rowData, undefined, undefined, parent )
  }
  ...
  if (!rowData.checked) {
    ...
    // rowData.children && handleSelected(selected, parent, 'cancel', rowData)
    // const childExistChecked = rowData.children && rowData.children.some(item => item.checked)
    // if (childExistChecked) {
    //   handleSelected(selected, rowData, 'cancel', rowData)
    // }
    parent.children.forEach(child => {
      if (child.value !== rowData.value && child.checked) {
        selected.push(child)
  ...
  /** 向上递归，一直到最顶层 */
  if (level !== 0) {
    changeAncestorsChecked(items, dataSource, parent, selected)
  }
}
...
const types = {
  ...
  'cancel': (selected: DataProps[], data: DataProps, type?: string, curr?: DataProps) => {
  ...
  // 将该节点放入selected中
  'undefined': (selected: DataProps[], data: DataProps, type?: string, curr?: DataProps, parent?: DataProps) => {
    selected.push(data)
    if (parent && parent.children) {
      types['parent'](selected, parent)
    }
    if (data.checked && data.children) {
      types['merge'](selected, data)
    }
  },
  // 顶层取消选中，直接退出
  'rootCancel': (selected: DataProps[], data: DataProps, type?: string, curr?: DataProps) => {}, 
  // 兄弟节点也是选中状态的话，放进去
  'parent': (selected: DataProps[], parent: DataProps) => {
    parent.children.forEach(child => {
      selected.every(sel => sel.value !== child.value && child.checked) && selected.push(child)
    })
  },
  // 该层子节点全都选中了，合并只保留父级节点数据
  'merge': (selected: DataProps[], data: DataProps) => {
 ...
 */
export const handleSelected = (selected: DataProps[], data: DataProps, type?: string, curr?: DataProps, parent?: DataProps) => {
  /** 先将该节点及其子孙后代数据扁平化 */
  let flatData = [data]
  ...
  types[String(type)](selected, data, type, curr, parent)
}